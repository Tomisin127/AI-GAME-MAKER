...
#then add sprite and collision shape as a child
	...
	if previous_object is RigidBody2D:
		...
		I AM  A 19 YEAR OLD BOY FROM NIGERIA(AFRICA) HAS INTREST
		IN GAME DEVELOPMENT , CODES IN C, C++, PHYTON, GDSCRIPT
		AND CURRENTLY STUDIES COMPUTER ENGINEERING AT OBAFEMI AWOLOWO UNIVERSITY
		I HAVE LAUNCHED TWO GAMES ON ANDROID PLAYSTORE [ FOCUS(SMASH GAME) AND ARENAGAMES2D ]
		
		CONTACT = +2348185842906
		        = tomisinraimi@gmail.com
		
		__DONATIONS AND SUPPORT__
		CRPYTOCURRENCIES
		BTC = 3FSYcTF2t7BwpW8CxU9qhYKfHQqgvCa3Qe
